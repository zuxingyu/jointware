...
public class KubernetesDispatcherService extends DispatcherService {
	...
	protected void generateParameter(String fullname, Map<String, Object> paramValues) throws Exception {
		...
		} else if (JavaUtils.isList(typename)) {
...
//			}
//		} 
		else if (JavaUtils.isMap(typename)) {
...
//		} 
	...
	protected Map<String, String> createParamsType(String kind) {
		try {
			return StringUtils.isNull(kind) ? new HashMap<String, String>()
					: new KubernetesModelParameterAnalyzer().getModelParameters(kind);
		} catch (Exception e) {
			return null;
		}
	}
	...
	protected Object createKindModel(String kind) throws Exception {
		return Class.forName(new KubernetesKindModelAnalyzer().getKindModel(kind)).newInstance();
	}
	...
	protected Class<?> getParamType(String typename) throws Exception {
		...
		if (JavaUtils.isMap(typename)) {
			...
			return List.class;
		} else if (JavaUtils.isSet(typename)){
			return Set.class;
		}else {
